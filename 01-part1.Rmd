...
To get started, click [here](https://moodle.gla.ac.uk/mod/resource/view.php?id=3370009) to download the data `crimedata.rds`. Alternatively, you can find the data in the Lab Section on [Moodle](https://moodle.gla.ac.uk/course/view.php?id=33832) and download it from there, by clicking on the file name. Once the file has downloaded, **save it in the same folder** as the `R` file you use to work on this lab (in `RStudio`). You can do this by clicking `Session > Set Working Directory > To Source File Location`. Alternatively, you can run the following line of code:

```{r load-data1, eval=TRUE}
